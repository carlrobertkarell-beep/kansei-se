...
class T(unittest.TestCase):
 ...
 def test_patient_app_loads_versioned_motion_library(self):
  self.p.goto(self.o+'/reda-2/');self.p.wait_for_function("()=>window.RedaMotionSpecs&&window.RedaFigures");spec=self.p.evaluate("RedaMotionSpecs.get('sit-to-stand.support')");self.assertEqual(spec['version'],1);self.assertEqual(spec['key'],'sit-to-stand.support');self.assertEqual(self.p.locator('script[src="motion-specs.js"]').count(),1);self.assertEqual(self.p.locator('script[src="figures.js"]').count(),1)
