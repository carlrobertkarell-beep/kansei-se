...
class T(unittest.TestCase):
 ...
 def test_lab_lists_graph_and_versioned_motion(self):
  self.assertIn('Övningsgraf',self.p.locator('h1').inner_text());self.assertGreater(self.p.locator('.node-btn').count(),20);self.assertEqual(self.p.locator('#motion svg').get_attribute('data-motion-version'),'1');self.assertLessEqual(self.p.evaluate('document.documentElement.scrollWidth'),391)
