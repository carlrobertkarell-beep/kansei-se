...
class T(unittest.TestCase):
 ...
 def test_progression_frame_saved_as_inactive_draft_and_stale_when_prescription_changes(self):
  ...
  for id,value in [('path-reps','19'),('pathDays','3'),('pathMinDays','7'),('pathEvidenceDays','14'),('pathFrom','2026-09-10'),('pathUntil','2026-10-22')]:self.p.locator('#'+id).fill(value)
  self.p.locator('#pathLabel').fill('Nästa fiktiva dos');self.p.locator('#pathReason').fill('Fiktiv klinisk motivering');self.p.locator('[data-add-step]').click();self.p.locator('#pathEffort').select_option('easy');
  ...
  self.p.locator('[data-save-path]').click();self.assertIn('Sparat ramutkast · 2 steg',self.p.locator('.progression-author').inner_text());self.save();d=self.p.evaluate("JSON.parse(localStorage.getItem('care-draft')).payload");self.assertEqual(d['progressionDraft']['mode'],'simulation-only');self.assertEqual(d['exercises'][0]['dose']['reps'],17);self.assertEqual(d['progressionDraft']['steps'][1]['plan']['exercises'][0]['dose']['reps'],19)
  self.p.locator('#goal').fill('Nytt fiktivt mål');self.assertIn('behöver förnyas',self.p.locator('.progression-author').inner_text());self.p.locator('[data-patient="b"]').click();self.p.wait_for_function("!document.querySelector('#careMode').disabled");self.assertNotIn('Sparat ramutkast',self.p.locator('.progression-author').inner_text())
