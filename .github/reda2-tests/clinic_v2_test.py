...
class T(unittest.TestCase):
 ...
 def test_security_files_have_required_guards(self):
  sql=(ROOT/'reda-2/supabase/migrations/20260909_001_secure_reda.sql').read_text();rpc=(ROOT/'reda-2/supabase/migrations/20260909_002_session_rpc.sql').read_text();fn=(ROOT/'reda-2/supabase/functions/invite-patient/index.ts').read_text();compact=''.join(fn.split());self.assertIn('enable row level security',sql);self.assertIn("(auth.jwt()->>'aal')='aal2'",sql);self.assertIn('revoke all on public.reda_profiles',sql);self.assertIn('reda_activate_plan',sql);self.assertIn('client_session_id',rpc);self.assertIn('on conflict(client_session_id)',rpc);self.assertIn("ctx.jwtClaims?.aal!=='aal2'",compact);self.assertIn('inviteUserByEmail',fn);self.assertNotIn('SUPABASE_SECRET_KEY',fn)
