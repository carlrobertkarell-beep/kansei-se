...
class T(unittest.TestCase):
 ...
 def test_mobile_and_desktop_proposal_layout(self):
  self.p.locator('[data-detail="a-2"]').click();self.p.locator('[data-approve]').wait_for();out=ROOT/'reda2-test-results';out.mkdir(exist_ok=True);self.p.screenshot(path=str(out/'dashboard-desktop.png'),full_page=True);self.p.set_viewport_size({'width':390,'height':844});self.assertTrue(self.p.evaluate('document.documentElement.scrollWidth<=innerWidth+1'));self.p.screenshot(path=str(out/'dashboard-mobile.png'),full_page=True)
