...
class Patient(RecoveryTests):
 ...
 def test_mobile_learning_card_has_readable_text_and_large_controls(self):
  self.p.goto(self.origin+'/reda-2/exercise-guide.html');self.p.locator('#movementLesson').scroll_into_view_if_needed();self.assertTrue(self.p.evaluate('document.documentElement.scrollWidth<=innerWidth+1'));self.assertGreaterEqual(self.p.locator('#lessonNext').bounding_box()['height'],52);self.assertGreaterEqual(self.p.locator('#lessonInstruction').evaluate('(el)=>parseFloat(getComputedStyle(el).fontSize)'),20)
