...
class Patient(RecoveryTests):
 ...
 def test_reduced_motion_keeps_start_still_and_can_show_an_endpoint(self):
  self.p.emulate_media(reduced_motion='reduce');self.p.reload();self.p.locator('#start').click();expect(self.p.locator('#motionPlay')).to_be_disabled();self.p.locator('#lessonGuided').click();self.p.locator('#movementLesson .lesson-stills summary').click();self.p.locator('#movementLesson [data-motion-frame="1"]').click();expect(self.p.locator('#lessonDetails')).not_to_be_visible();expect(self.p.locator('#motionPhase')).to_have_text('Slutläge');self.assertEqual(self.p.locator('#movementLesson').get_attribute('data-playing'),'false')
 ...
 def test_chair_parts_stop_and_return_to_the_same_training_action(self):
  self.p.goto(self.origin+'/reda-2/exercise-guide.html');self.p.locator('#start').click();self.p.locator('#lessonGuided').click();self.p.locator('#movementLesson [data-lesson-mode="watch"]').click();self.p.locator('#movementLesson [data-lesson-part="0"]').click();expect(self.p.locator('#lessonDetails')).not_to_be_visible();expect(self.p.locator('#movementLesson')).to_have_attribute('data-playing','false',timeout=9000);self.assertAlmostEqual(float(self.p.locator('#motion').get_attribute('data-position')),.23);expect(self.p.locator('[data-round="0"]')).to_be_visible();expect(self.p.locator('#sessionProgressText')).to_contain_text('0 av')
 ...
 def test_preview_starts_with_plan_and_completes_one_connected_session_without_saving(self):
  ...
  self.p.locator('#start').click();expect(self.p.locator('#pos')).to_have_text('1 av 3');self.p.locator('[data-round="0"]').click();self.p.locator('#closePlayer').click();expect(self.p.locator('#start')).to_have_text('Fortsätt passet');expect(self.p.locator('#todayExercises')).to_contain_text('1 omgång klar');self.shot('journey-resume.png');self.p.locator('#start').click();self.p.locator('#resumeExercise').click();expect(self.p.locator('[data-round="1"]')).to_be_visible();self.p.locator('[data-round="1"]').click();self.p.locator('#next').click()
  for n in [2,3]:
 ...
 def test_new_plan_does_not_inherit_old_plan_completion_in_overview(self):
  self.p.locator('[data-round="0"]').click();self.p.locator('[data-round="1"]').click();self.p.evaluate("const d=JSON.parse(localStorage.getItem('test-server'));d.plan.id='plan-2';d.plan.version=2;localStorage.setItem('test-server',JSON.stringify(d))");self.p.locator('#next').click();expect(self.p.locator('#player')).not_to_be_visible();expect(self.p.locator('#todayExercises .journey-row-status')).to_have_text('Visa övningen')
