...
class Patient(RecoveryTests):
 ...
 def test_chair_parts_stop_and_return_to_the_same_training_action(self):
  self.p.goto(self.origin+'/reda-2/exercise-guide.html');self.p.locator('#lessonGuided').click();self.p.locator('[data-lesson-mode="watch"]').click();self.p.locator('[data-lesson-part="0"]').click();expect(self.p.locator('#lessonDetails')).not_to_be_visible();expect(self.p.locator('#movementLesson')).to_have_attribute('data-playing','false',timeout=9000);self.assertAlmostEqual(float(self.p.locator('#motion').get_attribute('data-position')),.23);expect(self.p.locator('[data-round="0"]')).to_be_visible();expect(self.p.locator('#sessionProgressText')).to_contain_text('0 av')
 ...
 def test_small_and_large_phones_keep_primary_controls_on_screen(self):
  ...
  self.p.set_viewport_size({'width':844,'height':390});self.shot('phone-landscape.png');self.assertTrue(self.p.evaluate('document.documentElement.scrollWidth<=innerWidth+1'))
 def test_large_text_can_scroll_without_clipping_instructions_or_actions(self):
  self.p.set_viewport_size({'width':320,'height':640});self.p.evaluate("const els=[...document.querySelectorAll('#player,#player *')],sizes=els.map(e=>parseFloat(getComputedStyle(e).fontSize));els.forEach((e,i)=>e.style.fontSize=(sizes[i]*2)+'px')");self.p.locator('#lessonGuided').click();self.p.locator('#lessonNext').click();expect(self.p.locator('#lessonInstruction')).to_have_text('Sitt med stöd.');self.assertTrue(self.p.evaluate('document.documentElement.scrollWidth<=innerWidth+1'));self.p.locator('#lessonCloseDetails').click();self.p.locator('[data-round="0"]').click();expect(self.p.locator('#sessionProgressText')).to_contain_text('1 av 2')
