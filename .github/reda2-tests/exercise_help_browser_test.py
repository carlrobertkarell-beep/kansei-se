...
class Patient(RecoveryTests):
 ...
 def test_reduced_motion_allows_stills_and_saved_instruction(self):
  self.p.emulate_media(reduced_motion='reduce');self.p.reload();self.p.locator('#start').click();expect(self.p.locator('#motionPlay')).to_be_disabled();self.p.locator('[data-motion-frame="1"]').click();expect(self.p.locator('#motionPhase')).to_have_text('Slutläge');self.topic('execution');expect(self.p.locator('.coach-answer')).to_contain_text('Följ den ordinerade rörelsen.')
 def test_mobile_keyboard_focus_touch_targets_and_rendering(self):
  ...
  self.p.locator('#motionPlay').click();expect(self.p.locator('#motionPlay')).to_have_text('Pausa rörelsen');self.p.locator('#motionSlow').click();expect(self.p.locator('#motionSlow')).to_have_attribute('aria-pressed','true');self.p.locator('#closePlayer').click();expect(self.p.locator('#player')).to_be_hidden()
 def test_review_guide_all_five_families_and_simulated_case_are_isolated(self):
  ...
  for n in range(5):
   self.p.locator('#guideExercise').select_option(str(n));self.p.locator('[data-frame="1"]').click();expect(self.p.locator('#motion svg')).to_have_attribute('data-renderer-version','5');self.assertTrue(self.p.evaluate('document.documentElement.scrollWidth<=innerWidth+1'))
