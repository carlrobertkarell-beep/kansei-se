...
class Patient(RecoveryTests):
 ...
 def test_reduced_motion_allows_stills_and_saved_instruction(self):
  self.p.emulate_media(reduced_motion='reduce');self.p.reload();self.p.locator('#start').click();self.p.locator('#lessonGuided').click();self.p.locator('[data-lesson-mode="watch"]').click();expect(self.p.locator('#motionPlay')).to_be_disabled();self.p.locator('[data-motion-frame="1"]').click();expect(self.p.locator('#motionPhase')).to_have_text('Slutläge');self.topic('execution');expect(self.p.locator('.coach-answer')).to_contain_text('Följ den ordinerade rörelsen.')
 ...
 def test_review_guide_all_five_families_and_simulated_case_are_isolated(self):
  ...
  for n in range(5):
   self.p.locator('#guideExercise').select_option(str(n));self.p.locator('#lessonGuided').click();self.p.locator('[data-lesson-mode="watch"]').click();self.p.locator('.lesson-stills summary').click();self.p.locator('[data-frame="1"]').click();expect(self.p.locator('#motion svg')).to_have_attribute('data-renderer-version','5');self.assertTrue(self.p.evaluate('document.documentElement.scrollWidth<=innerWidth+1'))
 ...
 def test_direct_training_preserves_choice_on_side_change_without_recording_a_round(self):
  expect(self.p.locator('#movementLesson')).to_have_attribute('data-mode','overview');expect(self.p.locator('#motionStudio')).to_be_hidden();expect(self.p.locator('#motionSide')).to_be_visible();expect(self.p.locator('#motionSide')).to_have_text('Vänster sida');self.assertLess(self.p.locator('#dose').bounding_box()['y'],self.p.locator('#lessonReady').bounding_box()['y'])
  self.p.locator('#lessonReady').click();expect(self.p.locator('#movementLesson')).to_have_attribute('data-mode','practice');expect(self.p.locator('[data-round="0"]')).to_be_focused();expect(self.p.locator('#sessionProgressText')).to_contain_text('0 av');self.assertEqual(self.p.evaluate('helpRows.length'),0)
  self.p.locator('[data-round="0"]').click();expect(self.p.locator('#motionSide')).to_have_text('Höger sida');expect(self.p.locator('#movementLesson')).to_have_attribute('data-mode','practice');expect(self.p.locator('#motionStudio')).to_be_hidden();self.p.locator('#closePlayer').click();self.p.locator('#start').click();expect(self.p.locator('#movementLesson')).to_have_attribute('data-mode','practice')
  out=Path('reda2-test-results');out.mkdir(exist_ok=True);self.p.locator('#player').scroll_into_view_if_needed();self.p.screenshot(path=str(out/'flexible-practice-mobile.png'),full_page=True)
 def test_whole_demonstration_runs_once_and_can_be_left_for_training(self):
  self.p.locator('#lessonWhole').click();expect(self.p.locator('#movementLesson')).to_have_attribute('data-playing','true');expect(self.p.locator('#lessonInstruction')).to_be_hidden();expect(self.p.locator('#lessonNextPart')).to_be_hidden();expect(self.p.locator('#movementLesson')).to_have_attribute('data-playing','false',timeout=17000);expect(self.p.locator('#motionPlay')).to_have_text('Visa hela igen');self.assertAlmostEqual(float(self.p.locator('#motion').get_attribute('data-position')),0);expect(self.p.locator('#sessionProgressText')).to_contain_text('0 av')
  self.p.locator('#motionPlay').click();expect(self.p.locator('#movementLesson')).to_have_attribute('data-playing','true');self.p.locator('#lessonContinue').click();expect(self.p.locator('#movementLesson')).to_have_attribute('data-playing','false');expect(self.p.locator('[data-round="0"]')).to_be_focused();expect(self.p.locator('#sessionProgressText')).to_contain_text('0 av')
 def test_guidance_depth_can_change_and_reading_position_survives_side_change(self):
  self.p.locator('#lessonWhole').click();self.p.locator('#lessonGuided').click();expect(self.p.locator('#movementLesson')).to_have_attribute('data-playing','false');self.p.locator('#lessonNext').click();expect(self.p.locator('#lessonInstruction')).to_have_text('Sitt med stöd.');self.p.locator('[data-round="0"]').click();expect(self.p.locator('#lessonInstruction')).to_have_text('Sitt med stöd.');expect(self.p.locator('#motionSide')).to_have_text('Höger sida')
  self.p.locator('#lessonContinue').click();self.p.locator('#lessonGuided').click();expect(self.p.locator('#lessonInstruction')).to_have_text('Sitt med stöd.');expect(self.p.locator('#movementLesson')).to_have_attribute('data-playing','false')
 def test_guide_choices_are_per_exercise_and_only_in_page_memory(self):
  self.p.goto(self.origin+'/reda-2/exercise-guide.html');self.p.locator('#movementLesson').scroll_into_view_if_needed();out=Path('reda2-test-results');out.mkdir(exist_ok=True);self.p.screenshot(path=str(out/'flexible-entry-mobile.png'),full_page=True)
  self.p.locator('#lessonGuided').click();self.p.locator('#lessonNext').click();instruction=self.p.locator('#lessonInstruction').inner_text();self.p.locator('#guideExercise').select_option('1');expect(self.p.locator('#movementLesson')).to_have_attribute('data-mode','overview');self.p.locator('#guideExercise').select_option('0');expect(self.p.locator('#movementLesson')).to_have_attribute('data-mode','read');expect(self.p.locator('#lessonInstruction')).to_have_text(instruction)
  self.p.reload();expect(self.p.locator('#movementLesson')).to_have_attribute('data-mode','overview');self.p.locator('#lessonWhole').click();self.p.locator('#motionPlay').click();self.p.set_viewport_size({'width':1280,'height':1000});self.p.screenshot(path=str(out/'flexible-whole-desktop.png'),full_page=True)
