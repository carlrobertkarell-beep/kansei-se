...
class Patient(RecoveryTests):
 ...
 def test_logged_in_patient_can_retry_missing_plan_without_logging_in_again(self):
  self.p.evaluate("localStorage.setItem('no-plan','yes')");self.p.reload();expect(self.p.locator('#authError')).to_contain_text('Ingen aktiv plan');self.p.locator('.account-trigger').click();expect(self.p.locator('#logout')).to_be_visible();self.p.keyboard.press('Escape');self.p.evaluate("localStorage.removeItem('no-plan')");self.p.locator('#retryAccess').click();expect(self.p.locator('#app')).to_be_visible()
