...
class T(unittest.TestCase):
 ...
 def test_help_and_motion_controls_preserve_prescription(self):
  self.p.goto(self.o+'/reda-2/patient.html');self.p.locator('#start').click();before=self.p.locator('#dose').inner_text();self.p.locator('#lessonGuided').click();expect(self.p.locator('#lessonInstruction')).to_be_visible();self.p.locator('#lessonCloseDetails').click();self.p.locator('#openPlayerOptions').click();self.p.locator('.lesson-prescription > summary').click();expect(self.p.locator('.ei-instructions')).to_be_visible();self.p.locator('[data-close-sheet="playerOptions"]').click();self.p.locator('#lessonGuided').click();self.p.locator('#movementLesson .lesson-stills summary').click();self.p.locator('#movementLesson [data-motion-frame="1"]').click();expect(self.p.locator('#movementLesson [data-motion-frame="1"]')).to_have_attribute('aria-pressed','true');self.p.locator('#motionPlay').click();expect(self.p.locator('#movementLesson')).to_have_attribute('data-playing','true');self.p.locator('#motionSlow').click();expect(self.p.locator('#motionSlow')).to_have_attribute('aria-pressed','true');self.p.locator('#motionPlay').click();self.assertEqual(self.p.locator('#dose').inner_text(),before);expect(self.p.locator('[data-round="0"]')).to_be_enabled();self.shot('reda-intelligence-patient-guidance.png')
