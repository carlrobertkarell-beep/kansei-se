...
class Account(RecoveryTests):
 ...
 def settings(self):
  self.p.get_by_role('button',name='Öppna kontomenyn').click();self.p.get_by_role('button',name='Anpassa Reda',exact=False).click()
 def close_settings(self):self.p.get_by_role('button',name='Stäng inställningar',exact=True).click()
 def section(self,name):
  self.p.get_by_role('button',name='Tillbaka till kontomenyn').click();self.p.locator('.account-menu').get_by_role('button',name=name,exact=False).click()
 ...
 def test_large_text_single_account_navigation_fits_small_phone(self):
  self.p.set_viewport_size({'width':320,'height':640});self.settings();self.p.get_by_role('button',name='Större text',exact=True).click();expect(self.p.locator('html')).to_have_attribute('data-reda-text','large');self.shot('account-settings-large.png');self.assertTrue(self.p.evaluate('document.documentElement.scrollWidth<=innerWidth+1'));expect(self.p.locator('.settings-category')).to_have_count(0);self.section('Konto & integritet');expect(self.p.locator('#settingsPanel')).to_contain_text('Fiktiv testperson');expect(self.p.get_by_role('link',name='Kontakt och hjälp med kontot',exact=False)).to_have_attribute('href','/kontakt/');self.p.get_by_role('button',name='Tillbaka till kontomenyn').click();expect(self.p.locator('.account-menu')).to_be_visible();expect(self.p.locator('.account-menu [data-settings=profile]')).to_be_focused();expect(self.p.locator('.account-menu')).not_to_contain_text('Köp & abonnemang');self.p.keyboard.press('Escape');expect(self.p.locator('.account-trigger')).to_be_focused();expect(self.p.locator('.account-gear')).to_have_count(0)
 ...
 def test_open_settings_follow_preference_changes_from_another_tab(self):
  self.settings();other=self.c.new_page();other.goto(self.origin+'/reda-2/exercise-guide.html');other.get_by_role('button',name='Öppna kontomenyn').click();other.get_by_role('button',name='Anpassa Reda',exact=False).click();other.get_by_role('button',name='Ljust',exact=True).click();expect(self.p.locator('html')).to_have_attribute('data-reda-theme','light');expect(self.p.get_by_role('button',name='Ljust',exact=True)).to_have_attribute('aria-pressed','true');other.close()
 def test_desktop_account_back_navigation_and_invalid_stored_preference(self):
  self.p.evaluate("localStorage.setItem('reda-presentation-v1',JSON.stringify({theme:'invalid',text:'invalid',motion:'invalid'}))");self.p.reload();expect(self.p.locator('html')).to_have_attribute('data-reda-theme','dark');self.p.set_viewport_size({'width':1280,'height':900});self.settings();self.shot('account-settings-desktop.png');self.section('Hjälp & kontakt');expect(self.p.locator('#settingsPanel')).to_contain_text('Snabba svar');self.p.get_by_text('Kan jag pausa ett pass?',exact=True).click();expect(self.p.locator('#settingsPanel')).to_contain_text('Fortsätt passet');expect(self.p.get_by_label('Inställningskategori',exact=True)).to_have_count(0)
 def test_help_opens_exercise_selection_without_starting_a_session(self):
  self.p.goto(self.origin+'/reda-2/exercise-guide.html');self.settings();self.section('Hjälp & kontakt');self.shot('account-help-dark.png');self.p.get_by_role('button',name='Välj övning att få hjälp med',exact=False).click();expect(self.p.locator('.patient-settings')).not_to_be_visible();expect(self.p.locator('#program')).to_be_visible();expect(self.p.locator('#player')).not_to_be_visible();expect(self.p.locator('#programExercises button').first).to_be_focused();self.p.locator('[data-tab=activity]').click();expect(self.p.locator('#demoLog li')).to_have_count(0)
