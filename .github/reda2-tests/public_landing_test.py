...
class T(unittest.TestCase):
 ...
 def test_public_reda_landing(self):
  self.p.goto(self.o+'/reda-rehab/');self.assertIn('Din plan',self.p.locator('h1').inner_text());self.assertIn('Försäljningen har inte öppnat',self.p.locator('.care-options').inner_text());self.assertNotIn('2 490',self.p.locator('body').inner_text());self.p.get_by_text('Ändras planen när en övning känns lätt?',exact=True).click();self.assertIn('automatiskt',self.p.locator('body').inner_text().lower());self.assertEqual(self.p.locator('link[rel="canonical"]').get_attribute('href'),'https://www.kansei.se/reda-rehab/');self.assertLessEqual(self.p.evaluate('document.documentElement.scrollWidth'),391)
 ...
 def test_responsive_preview_and_motion_controls(self):
  self.p.emulate_media(reduced_motion='reduce');self.p.goto(self.o+'/reda-rehab/');self.p.locator('[data-view="exercise"]').click();self.assertEqual(self.p.locator('#play-motion').inner_text(),'Visa rörelsen');self.p.locator('[data-frame="0.52"]').click();self.assertEqual(self.p.locator('[data-frame="0.52"]').get_attribute('aria-pressed'),'true');self.assertLessEqual(self.p.evaluate('document.documentElement.scrollWidth'),390)
