...
class T(unittest.TestCase):
 ...
 def test_coming_soon_and_support_without_javascript(self):
  ...
  for route in ['/','/rehabilitering/']:
   self.p.goto(self.o+route);self.p.locator('#planKort').scroll_into_view_if_needed();self.assertIn('kommer snart',self.p.locator('#planKort').inner_text().lower());self.assertEqual(self.p.locator('#planKort .reda-portal-link').get_attribute('href'),'/reda/')
