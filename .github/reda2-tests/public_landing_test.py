...
class T(unittest.TestCase):
 ...
 def test_public_reda_landing(self):
  self.p.goto(self.o+'/reda-rehab/');self.assertIn('Din plan',self.p.locator('h1').inner_text());self.p.get_by_text('Ändras planen när en övning känns lätt?',exact=True).click();self.assertIn('automatiskt',self.p.locator('body').inner_text().lower());self.assertEqual(self.p.locator('link[rel="canonical"]').get_attribute('href'),'https://www.kansei.se/reda-rehab/');self.assertLessEqual(self.p.evaluate('document.documentElement.scrollWidth'),391)
