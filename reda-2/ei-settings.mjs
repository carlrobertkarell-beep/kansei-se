...
export function mountEISettings(host,{api,onBusy,onImport}){
 ...
 async function load(message=''){
  ...
  try{
   ...
   form.onsubmit=async e=>{
    ...
    busy=true;onBusy?.(true);clinical?.setBlocked(true);form.querySelectorAll('button,input').forEach(el=>el.disabled=true);q('[data-import-patient]').disabled=true;
    ...
    try{
     ...
     busy=false;onBusy?.(false);clinical?.setBlocked(false);
 ...
 load();return {destroy(){clinical?.destroy();alive=false;++ticket;host.replaceChildren()},isBusy:()=>busy};
}
