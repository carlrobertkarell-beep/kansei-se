...
export const caseLabels={changed_symptoms:'Förändrade besvär eller funktion',changed_environment:'Ändrade förutsättningar',requested_contact:'Patienten önskar kontakt',execution_help:'Hjälp med utförandet',training_barrier:'Hinder i träningsvardagen'};
...
export function propose(data){
 ...
 if(hasNew||cases.length&&p.followup_status!=='waiting'){
  if(cases.some(c=>c.code==='changed_symptoms'))return follow('Följ upp den förändrade återkopplingen','Följ upp patientens rapport om förändrade besvär eller funktion och bedöm behovet av fortsatt åtgärd.','Patientens svar beskriver förändrade besvär eller att besvären gjorde passet svårt. Bedöm det ursprungliga svaret innan ärendet avslutas.');
  const priority={changed_symptoms:0,changed_environment:1,requested_contact:2,execution_help:3,training_barrier:4};
  const ordered=[...cases].sort((a,b)=>(priority[a.code]??5)-(priority[b.code]??5)||Date.parse(a.created_at)-Date.parse(b.created_at));
  const immediate=ordered[0]?.reflection?ordered[0]:null;
  if(immediate){const r=immediate.reflection,a=r.answers,which=r.exercise_name?' Övning: '+r.exercise_name+'.':'',version=' Återkopplingen gäller plan v'+r.plan_version+'.';
   if(a.barrier==='time')return {...follow('Gör planen möjlig att hinna med','Granska om passets omfattning och veckoplan passar patientens vardag. Förbered vid behov ett kortare upplägg att gå igenom med patienten.'+which,'Patienten har angett att tiden inte räckte till.'+version),adjustPlan:true};
   if(a.barrier==='energy')return {...follow('Stäm av ork och passets omfattning','Stäm av vad den minskade orken beror på och bedöm passets omfattning innan en ändring görs.'+which,'Patienten har angett att orken inte räckte.'+version),adjustPlan:true};
   if(a.barrier==='equipment')return {...follow('Anpassa till tillgänglig utrustning','Stäm av vilken utrustning och plats patienten har. Granska en lämplig biblioteksvariant och dos innan planen ändras.'+which,'Patienten saknade utrustning eller plats under passet.'+version),adjustPlan:true};
   if(a.barrier==='execution')return ask('Hjälp med den aktuella instruktionen','Hej! Du var osäker på utförandet.'+which+' Vilken del behöver vi förtydliga? Svara här så hjälper jag dig vidare.','Patienten har redan angett utförandet som hinder.'+version,follow('Gå igenom instruktionen','Gå igenom planens instruktion tillsammans med patienten.'+which,'Osäkerhet om utförandet är rapporterad.'+version));
   if(a.barrier==='other')return ask('Ta reda på vad som hindrade passet','Hej! Du angav att något gjorde passet svårt.'+which+' Vad behöver vi hjälpa dig med?','Patienten har angett ett annat hinder.'+version,follow('Stäm av hindret','Stäm av vad som gjorde passet svårt.'+which,'Patienten har angett ett annat hinder.'+version));
  }
