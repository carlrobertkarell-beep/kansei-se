...
const root=document.getElementById('examples');examples.forEach(([key,name,cue,review],n)=>{
 ...
 root.append(card);let stop=null;const stage=card.querySelector('.stage'),play=card.querySelector('.play');
 const pause=()=>{stop?.();stop=null;play.textContent='Visa rörelsen'};
 play.onclick=()=>{if(stop){pause();return}if(matchMedia('(prefers-reduced-motion: reduce)').matches){play.textContent='Välj ett stilla läge nedan';return}stop=F.animate(stage,key,{seconds:Number(card.querySelector('select').value)});play.textContent='Pausa rörelsen';card.querySelectorAll('[data-frame]').forEach(b=>b.setAttribute('aria-pressed','false'))};
 card.querySelector('select').onchange=()=>{if(stop){pause();play.click()}};
 card.querySelectorAll('[data-frame]').forEach(b=>b.onclick=()=>{pause();stage.innerHTML=F.svg(key,Number(b.dataset.frame),'simultaneous',name);card.querySelectorAll('[data-frame]').forEach(x=>x.setAttribute('aria-pressed',String(x===b)))});
});})();
