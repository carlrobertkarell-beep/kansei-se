...
export function createMovementLesson(host){
 ...
 function renderPlayback(s){
  ...
  const label=media.matches?'Stilla visning':s.playing?'Pausa':s.complete?'Visa igen':s.progress>0?'Fortsätt':view==='whole'?'Visa rörelsen':'Visa delen';
  play.innerHTML=glyph(s.playing?'pause':'play')+`<span>${label}</span>`;
  play.setAttribute('aria-label',s.playing?'Pausa rörelsen':label);
 ...
 play.onclick=()=>{silence();if(media.matches)return;if(playback.state().playing)playback.pause();else{if(customStill){view='whole';customStill=false;playback.select(wholeLessonSequence(key));$('#motionPhase').textContent='Hela rörelsen'}playback.play()}};
 ...
 $('#lessonExpand').onclick=()=>{$('#focusTitle').textContent=exercise.name;$('.focus-body').append(visual);$('#lessonExpand').hidden=true;zoom.showModal()};
 $('#lessonCloseFocus').onclick=()=>zoom.close();zoom.addEventListener('close',()=>{pause();anchor.after(visual);$('#lessonExpand').hidden=false;$('#lessonExpand').focus({preventScroll:true})});
 ...
 return {pause,showFrame,openDetails,position:()=>playback.state().position,
  setExercise(x,{motionKey,exerciseSide,contextId}={}){
   ...
   pause();remember();if(details.open)details.close();if(zoom.open)zoom.close();identity=next;exercise=x;key=nextKey;side=nextSide;instructions=lessonInstructions(x);segments=lessonSegments(key);focus=null;view='whole';customStill=false;
   ...
   const steps=x.instructions?.length?x.instructions:x.steps?.length?x.steps:[x.instruction||x.cue];$('#lessonCue').textContent=steps.find(t=>typeof t==='string'&&t.trim())||'Öppna din instruktion';
