...
export function createMovementLesson(host,{idPrefix='',playLabel='Visa'}={}){
 ...
 function choosePart(i){pause();view='parts';part=i;customStill=false;focus=null;const s=segments[part];$('#motionPhase').textContent=s.label;playback.select(s);details.close();if(!reduced())playback.play();play.focus({preventScroll:true})}
 ...
 document.addEventListener('visibilitychange',visibility);media.addEventListener('change',reduce);window.addEventListener('reda:preferences',reduce);speech?.addEventListener('voiceschanged',voices);voices();
 return {pause,showFrame,openDetails,position:()=>playback.state().position,
  ...
  destroy(){pause();if(details.open)details.close();if(zoom.open)zoom.close();document.removeEventListener('visibilitychange',visibility);media.removeEventListener('change',reduce);window.removeEventListener('reda:preferences',reduce);speech?.removeEventListener('voiceschanged',voices)}
 };
}
