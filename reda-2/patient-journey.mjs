...
export function renderExerciseOverview(host,{exercises,progress=[],onPreview}){
 ...
 exercises.forEach((x,i)=>{
  const p=progress[i],status=p?.status==='completed'?'Klar':p?.status==='skipped'?(p.roundsDone?`${p.roundsDone} ${p.roundsDone===1?'omgång klar':'omgångar klara'} · resten överhoppade`:'Överhoppad'):p?.roundsDone?`${p.roundsDone} ${p.roundsDone===1?'omgång klar':'omgångar klara'}`:'';
