import {durationHint,planChanges,changesHTML,contactHint} from './patient-focus.mjs?v=20260920-1';
let displayedChanges=[],contactSupport=null,contactMessages=null;
import {mountSessionOverview} from './session-overview.mjs?v=20260918-1';
import {sessionOverview,hasNoRecordedRounds} from './session-overview-model.mjs?v=20260918-1';
...
let checkin=null;
import {mountPatientSupport} from './support-workflow.mjs?v=20260920-focus1'
let support=null;
import {mountActivityLog} from './activity-log.mjs?v=20260918-1'
let activity=null;
...
import {mountPatientMessages} from './patient-messages.mjs?v=20260920-focus1'
let messages=null;
...
// Bring due answers beside the next-pass action without duplicating forms.
$('patientActionArea').prepend($('reflectionPrompt'),$('responsePrompt'));
function renderContactHint(){const hint=contactHint(contactSupport,contactMessages),b=$('patientContactShortcut');b.hidden=!hint;if(hint){b.dataset.target=hint.kind;b.querySelector('strong').textContent=hint.title;b.querySelector('span').textContent=hint.detail}}
$('patientContactShortcut').onclick=()=>{const target=$('patientContactShortcut').dataset.target==='messages'?$('patientMessages'):$('patientSupport').hidden?$('patientMessages'):$('patientSupport');const d=target.querySelector('details');if(d)d.open=true;target.tabIndex=-1;target.focus({preventScroll:true});target.scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})};
...
const passOverview=mountSessionOverview($('player'),{
 read:()=>session?{exercises:exercises(),session,index,planId:plan.id,version:plan.version,optionLabel:session.optionId?plan.payload.planOptions?.items?.find(o=>o.id===session.optionId)?.label||'Godkänt alternativ':''}:null,
 isBusy:()=>marking||finishing||coach?.isBusy(),
 onOpen:()=>{pauseMotion();playerShell.closePanels()},
 onSelect:target=>{if(!session||session.completedAt||finishing||marking||!Number.isInteger(target)||target<0||target>=exercises().length)return;playerShell.endRest();index=target;showExercise();if(unsynced)persist();$('exName').focus({preventScroll:true});$('player').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})},
 onPause:()=>pause(),onFinish:()=>finishSession()
});
function renderToday(){
 $('patientDuration').textContent=durationHint(exercises());$('patientPlanChanges').innerHTML=changesHTML(displayedChanges,plan.version);
 renderOptions();$('exerciseCount').textContent=exercises().length;const view=todayState(plan,state.sessions);$('todayCard').dataset.dayState=view.kind;$('start').classList.toggle('primary',view.kind!=='done');$('start').classList.toggle('soft',view.kind==='done');$('todayTitle').textContent=view.title;$('todayDetail').textContent=view.detail;
 renderExerciseOverview($('todayExercises'),{exercises:exercises(),progress:session&&sessionSourcePlanId===plan.id&&(session.optionId||null)===(selectedOption||null)?session.progress:[],onPreview:x=>preview.open(x,{contextId:plan.id+':'+plan.version})});renderPreparation($('todayPreparation'),exercises());
...
function renderCare(){
 try{
  ...
  const summary=document.querySelector('.hero .notice summary');if(summary)summary.textContent=next?'Mål och uppföljning · '+next.dateLabel:'Mål och uppföljning';
...
function doseLabel(x){return x?.dose?.label||[x?.dose?.sets&&`${x.dose.sets} ${Number(x.dose.sets)===1?'omgång':'omgångar'}`,x?.dose?.reps&&`${x.dose.reps} ${Number(x.dose.reps)===1?'repetition':'repetitioner'}`].filter(Boolean).join(' · ')||'Följ ordinationen'}
...
function renderHome(){if($('patientDate'))$('patientDate').textContent=new Intl.DateTimeFormat('sv-SE',{weekday:'long',day:'numeric',month:'long',timeZone:'Europe/Stockholm'}).format(new Date());if($('patientStart'))$('patientStart').innerHTML=startHTML(state,planMessage);const xs=plan.payload.exercises||[];$('patientName').textContent=state.patient.display_name;$('version').textContent=plan.version;$('exerciseCount').textContent=xs.length;$('todayMeta').textContent=plan.payload?.schedule?.days?.length?`${plan.payload.schedule.days.length} planerade pass/vecka`:'Enligt din plan';$('intro').textContent=plan.payload?.goal?`Målet med den här perioden: ${plan.payload.goal}`:'Det här är den plan du och din behandlare har gått igenom.';renderCare();renderIntelligence();const helpPlan=plan;mountPlanHelp($('planHelp'),{api,plan,isCurrent:()=>plan===helpPlan,onResponse:()=>document.querySelector('[data-tab=follow]').click()});$('programList').innerHTML=xs.map((x,i)=>`<div class="exercise program-exercise"><div class="exercise-topline"><span>${String(i+1).padStart(2,'0')}</span><span>${esc(x.challenge||'Individuell nivå')}</span></div><h3>${esc(x.name)}</h3><div class="sub">${esc(x.variantLabel||'')} · ${esc(doseLabel(x))}</div>${whyBlock(x)}<button type="button" class="btn ghost" data-program-preview="${i}">Visa övningen</button></div>`).join('');$('programList').querySelectorAll('[data-program-preview]').forEach(b=>b.onclick=()=>preview.open(xs[Number(b.dataset.programPreview)],{contextId:plan.id+':'+plan.version}));renderHistory();renderToday()}
function renderHistory(){const rows=state.sessions||[];$('history').innerHTML=rows.length?rows.map(s=>`<div class="exercise"><b>${s.completed_at&&hasNoRecordedRounds(s.payload?.exercises)?'Avslutat utan registrerade omgångar':s.status==='completed'?'Genomfört':s.status==='partial'?'Delvis genomfört':s.status==='started'?'Påbörjat':'Planerad vila'}</b><div class="sub">${new Date(s.started_at).toLocaleDateString('sv-SE')} · plan v${s.plan_version}${s.payload?.optionId?' · Godkänt alternativ: '+esc((s.plan_id===plan.id?plan.payload.planOptions?.items?.find(o=>o.id===s.payload.optionId)?.label:null)||s.payload.optionId):''}</div></div>`).join(''):'<p class="muted">Ingen träningshistorik ännu.</p>'}
async function loadPlanNotice(){planMessage='';$('planReceiptStatus').hidden=true;if(api.openPatientPlan)try{const h=await api.openPatientPlan(plan.id);planMessage=h.message||''}catch{$('planReceiptStatus').hidden=false;$('planReceiptStatus').textContent='Planen visas, men öppningskvittot kunde inte bekräftas. Försök uppdatera sidan när anslutningen fungerar.'}}
async function bootstrap(){
 const prior=state;state=await api.patientBootstrap();displayedChanges=prior?.patient.id===state.patient.id?planChanges(prior.plan,state.plan):[];contactSupport=null;if(prior?.patient.id!==state.patient.id){contactMessages=null;messages?.destroy();messages=null}renderContactHint();accountUI.update({name:state.patient.display_name,signedIn:true});plan=state.plan;selectedOption=null;await loadPlanNotice();
...
async function start(){preview.close();const xs=exercises();if(!xs.length||starting||recoveryBlocked||reflectionForm.isBusy()||coach?.isBusy())return;starting=true;$('start').disabled=true;try{if(session?.completedAt&&unsynced){$('sync').textContent='Synka det avslutade passet innan du börjar ett nytt.';return}if(!session||session.completedAt){const fresh=await api.patientBootstrap();let checked=false;if(fresh.plan.id!==plan.id){if(pendingEvaluation){checked=await evaluateNext()!==false;if(!checked){$('sync').textContent='Nästa steg kunde inte kontrolleras. Försök igen innan du startar ett nytt pass.';return}}if(!checked||plan.id!==fresh.plan.id){await bootstrap();$('sync').textContent='Din behandlare har uppdaterat planen. Läs den nya versionen innan du startar passet.';return}}if(!checked&&await evaluateNext()===false){$('sync').textContent='Nästa steg kunde inte kontrolleras. Försök igen innan du startar ett nytt pass.';return}session=null;session=S.create(exercises(),crypto.randomUUID(),new Date().toISOString());sessionSourcePlanId=plan.id;if(selectedOption)session.optionId=selectedOption;index=0;await sync('started')} $('start').textContent='Fortsätt passet';$('sessionReflection').hidden=true;$('player').classList.remove('hidden');$('app').dataset.playerOpen='true';renderOptions();showExercise();$('exName').focus({preventScroll:true});$('player').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})}catch(e){$('sync').textContent=e.message||'Planen kunde inte kontrolleras. Försök igen.'}finally{starting=false;$('start').disabled=recoveryBlocked}}
function showExercise(){pauseMotion();const total=session.progress.reduce((n,p,i)=>n+sets(exercises()[i]),0),done=session.progress.reduce((n,p)=>n+p.roundsDone,0);if($('sessionProgress')){$('sessionProgress').max=total||1;$('sessionProgress').value=done;$('sessionProgressText').textContent=done+' av '+total+' omgångar registrerade'}const x=exercises()[index],p=session.progress[index];$('pos').textContent=`${index+1} av ${exercises().length}`;$('exName').textContent=x.name;$('dose').textContent=doseLabel(x)+(x.side==='both'?' per sida':x.side==='left'?' · vänster':x.side==='right'?' · höger':'')+(x.prescribedLoad?' · '+x.prescribedLoad:'');$('patientExerciseCues').innerHTML='<span><small>Sida</small><strong>'+esc(sideLabels[currentSide(x)]||'Enligt övningen')+'</strong></span><span><small>Tempo</small><strong>'+esc(x.prescription?.tempo||(Number.isFinite(x.dose?.tempo)?x.dose.tempo+' sek per repetition':'Tempo ej angivet'))+'</strong></span>';$('instruction').textContent=instruction(x);const complete=p.roundsDone>=sets(x)||p.status==='skipped',round=S.rounds(x)[p.roundsDone],priorRound=S.rounds(x)[p.roundsDone-1];$('playerStep').textContent=complete?(p.status==='skipped'?'Övningen överhoppad':'Övningen är klar'):priorRound&&priorRound.side!==round?.side?'Byt till '+(sideLabels[round.side]||'nästa sida').toLowerCase():'Din tur att träna';$('playerCurrentRound').textContent=complete?p.roundsDone+' av '+sets(x)+' omgångar registrerade':'Omgång '+round.set+(x.side==='both'?' på den här sidan':'')+' av '+(x.dose?.sets||1);$('playerConcern').hidden=true;$('exerciseWhy').innerHTML=whyBlock(x);renderExerciseHelp(x);$('rounds').innerHTML=S.rounds(x).map((r,i)=>`<button class="btn ${i<p.roundsDone?'primary':'soft'} round" data-round="${i}" ${i!==p.roundsDone?'disabled':''}>${i<p.roundsDone?'✓ ':''}Omgång ${r.set} klar${r.side==='left'?' · vänster':r.side==='right'?' · höger':''}</button>`).join('');document.querySelectorAll('[data-effort]').forEach(b=>{b.setAttribute('aria-pressed',String(b.dataset.effort===p.feedback));b.onclick=async()=>{if(session.completedAt||finishing||marking)return;p.feedback=b.dataset.effort;showExercise();await sync('partial')}});lesson.setExercise(x,{motionKey:motionKey(x),exerciseSide:currentSide(x),contextId:plan.id+':'+plan.version+':'+(session.optionId||'default')});coach?.setExercise({plan:sessionPlan(),exercise:x,sessionId:session.id});document.querySelectorAll('[data-round]').forEach(b=>b.onclick=()=>markRound(Number(b.dataset.round)));$('player').dataset.complete=String(p.roundsDone>=sets(x)||p.status==='skipped');$('next').disabled=p.roundsDone<sets(x)&&p.status!=='skipped';$('next').textContent=index===exercises().length-1?(session.progress.some(p=>p.status!=='completed'&&p.status!=='skipped')?'Se passöversikt':'Avsluta pass'):'Nästa övning';$('skip').disabled=p.status==='completed'}
async function markRound(i){if(marking||finishing||session?.completedAt||coach?.isBusy()||i!==session.progress[index].roundsDone)return;marking=true;try{const before=session.progress[index].roundsDone;session=S.mark(session,index,i);showExercise();if(session.progress[index].roundsDone>before&&session.progress[index].roundsDone<sets(exercises()[index]))playerShell.startRest(Number(exercises()[index].dose?.rest));if($('player').dataset.resting!=='true')($('next').disabled?$('rounds').querySelector('button:not(:disabled)'):$('next'))?.focus({preventScroll:true});await sync('partial')}finally{marking=false}}
async function skip(){if(!session||session.completedAt||finishing||marking||coach?.isBusy()||session.progress[index].status==='completed')return;playerShell.closePanels();playerShell.endRest();const p=session.progress[index];p.status='skipped';showExercise();await sync('partial')}
async function next(){
 if(!session||session.completedAt||finishing||marking||coach?.isBusy())return;
 const p=session.progress[index];if(p.status!=='completed'&&p.status!=='skipped')return;
 playerShell.closePanels();playerShell.endRest();
 if(index<exercises().length-1){index++;showExercise();$('exName').focus({preventScroll:true});$('player').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});if(unsynced)persist();return}
 if(session.progress.some(p=>p.status!=='completed'&&p.status!=='skipped')){passOverview.open();return}
 await finishSession();
}
async function finishSession(){
 if(!session||session.completedAt||finishing||marking||coach?.isBusy())return;
 const overview=sessionOverview(exercises(),session);if(!overview.canFinish){$('sync').textContent='Passet kunde inte avslutas. Kontrollera din plan och försök igen.';return}
 finishing=true;pauseMotion();playerShell.endRest();$('next').disabled=true;
 const all=overview.allCompleted,finishedId=session.id;
 try{
  const saved=await sync(all?'completed':'partial',new Date().toISOString());
  playerShell.closePanels();$('player').classList.add('hidden');$('app').dataset.playerOpen='false';$('start').textContent='Starta nytt pass';
  if(saved){try{const prior=state;state=await api.patientBootstrap();displayedChanges=prior?.patient.id===state.patient.id?planChanges(prior.plan,state.plan):[];plan=state.plan;if(prior?.plan.id!==plan.id)await loadPlanNotice();renderHome();barriers?.refresh();$('sync').textContent=all?'Passet är sparat och synkat':overview.done?'Passet är sparat som delvis genomfört':'Passet är avslutat utan registrerade omgångar';openReflection(pendingReflections(plan,state.sessions,state.reflections).find(x=>x.client_session_id===finishedId))}catch{$('sync').textContent='Passet är sparat, men historiken kunde inte uppdateras.'}}
  else $('retrySync').focus({preventScroll:true});
 }finally{finishing=false}
}
async function pause(){if(!session||session.completedAt||finishing||marking||coach?.isBusy())return;playerShell.closePanels();$('player').classList.add('hidden');$('app').dataset.playerOpen='false';pauseMotion();renderToday();$('start').textContent='Fortsätt passet';$('start').focus({preventScroll:true});await sync(session.progress.some(p=>p.roundsDone||p.status==='skipped')?'partial':'started')}
...
$('retrySync').onclick=retry;window.addEventListener('beforeunload',e=>{if(finishing||(unsynced&&!durable)||reflectionForm.isBusy()||checkin?.isBusy()||barriers?.isBusy()||coach?.isBusy()){e.preventDefault();e.returnValue=''}});
...
coach=createExerciseCoach($('exerciseCoach'),{api,onDemonstrate:(frame,focus,scroll=false)=>{lesson.showFrame(frame===null?lesson.position():frame,focus);if(scroll){playerShell.closePanels();$('motionStudio').focus({preventScroll:true});$('motionStudio').scrollIntoView({block:'center',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})}},onSaved:()=>{support?.refresh();barriers?.refresh()},onBusy:value=>{
