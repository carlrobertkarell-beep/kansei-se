...
function pauseMotion(){stopMotion?.();stopMotion=null;motionPlaying=false;$('motionPlay').textContent=matchMedia('(prefers-reduced-motion: reduce)').matches?'Stilla visning':'Visa rörelsen'}
function motionFrame(t){pauseMotion();const x=exercises()[index];if(!x)return;$('motion').innerHTML=F.svg(motionKey(x),t,x.side||'simultaneous',x.name);document.querySelectorAll('[data-motion-frame]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.motionFrame)===t)))}
function playMotion(){if(motionPlaying){pauseMotion();return}const x=exercises()[index];if(!x)return;stopMotion=F.animate($('motion'),motionKey(x),{seconds:(x.dose?.tempo||5)*(motionSlow?2:1),side:x.side});motionPlaying=true;$('motionPlay').textContent='Pausa rörelsen';document.querySelectorAll('[data-motion-frame]').forEach(b=>b.setAttribute('aria-pressed','false'))}
function renderExerciseHelp(x){const h=I.exercise(plan.payload,x),expanded=helpExpanded??plan.payload.presentation!=='trained';$('exerciseHelp').innerHTML='<button type="button" class="ei-help-toggle" id="helpToggle" aria-expanded="'+expanded+'">'+(expanded?'Visa kortare instruktion':'Visa steg för steg')+'</button>'+(expanded?'<p class="ei-setup">'+h.setup.map(esc).join(' · ')+'</p><ol class="ei-instructions">'+h.steps.map(s=>'<li>'+esc(s)+'</li>').join('')+'</ol>':'')+(h.range?'<p class="ei-setup">Ordinerat rörelseomfång: '+esc(h.range)+'</p>':'')+(h.rest!==null?'<p class="ei-setup">Ordinerad vila: '+h.rest+' sekunder mellan omgångarna.</p>':'');$('helpToggle').onclick=()=>{helpExpanded=!expanded;renderExerciseHelp(x)}}
function renderCare(){
 try{
...
async function start(){const xs=exercises();if(!xs.length||starting||recoveryBlocked||reflectionForm.isBusy())return;starting=true;$('start').disabled=true;try{if(session?.completedAt&&unsynced){$('sync').textContent='Synka det avslutade passet innan du börjar ett nytt.';return}if(!session||session.completedAt){if(await evaluateNext()===false){$('sync').textContent='Nästa steg kunde inte kontrolleras. Försök igen innan du startar ett nytt pass.';return}session=S.create(exercises(),crypto.randomUUID(),new Date().toISOString());index=0;await sync('started')} $('start').textContent='Fortsätt passet';$('sessionReflection').hidden=true;$('player').classList.remove('hidden');showExercise()}finally{starting=false;$('start').disabled=recoveryBlocked}}
function showExercise(){pauseMotion();const x=exercises()[index],p=session.progress[index];$('pos').textContent=`${index+1} av ${exercises().length}`;$('exName').textContent=x.name;$('dose').textContent=doseLabel(x)+(x.side==='both'?' per sida':x.side==='left'?' · vänster':x.side==='right'?' · höger':'')+(x.prescribedLoad?' · '+x.prescribedLoad:'');$('instruction').textContent=instruction(x);$('exerciseWhy').innerHTML=whyBlock(x);renderExerciseHelp(x);$('rounds').innerHTML=S.rounds(x).map((r,i)=>`<button class="btn ${i<p.roundsDone?'primary':'soft'} round" data-round="${i}" ${i!==p.roundsDone?'disabled':''}>${i<p.roundsDone?'✓ ':''}Omgång ${r.set}${r.side==='left'?' · vänster':r.side==='right'?' · höger':''}</button>`).join('');document.querySelectorAll('[data-effort]').forEach(b=>{b.setAttribute('aria-pressed',String(b.dataset.effort===p.feedback));b.onclick=async()=>{if(session.completedAt)return;p.feedback=b.dataset.effort;showExercise();await sync('partial')}});const key=motionKey(x);$('motion').innerHTML=F?.svg?F.svg(key,0,x.side||'simultaneous',x.name):'↕';document.querySelectorAll('[data-motion-frame]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.motionFrame==='0')));document.querySelectorAll('[data-round]').forEach(b=>b.onclick=()=>markRound(Number(b.dataset.round)));$('next').disabled=p.roundsDone<sets(x)&&p.status!=='skipped';$('next').textContent=index===exercises().length-1?'Avsluta pass':'Nästa övning'}
...
$('motionPlay').onclick=playMotion;$('motionSlow').onclick=()=>{motionSlow=!motionSlow;$('motionSlow').setAttribute('aria-pressed',String(motionSlow));if(motionPlaying){pauseMotion();playMotion()}};document.querySelectorAll('[data-motion-frame]').forEach(b=>b.onclick=()=>motionFrame(Number(b.dataset.motionFrame)));document.addEventListener('visibilitychange',()=>{if(document.hidden)pauseMotion()});
