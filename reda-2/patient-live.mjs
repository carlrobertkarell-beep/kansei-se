...
import {createExercisePreview,renderExerciseOverview,renderPreparation} from './patient-journey.mjs?v=4';
...
const accountUI=createPatientAccount({header:document.querySelector('.patient-app>.top'),logout:$('logout'),onExerciseHelp:()=>{if(playerBusy()||!plan)return;document.querySelector('[data-tab=program]').click();$('programList').querySelector('[data-program-preview]')?.focus({preventScroll:true})}});
let coach=null,marking=false,finalizing=false;
const preview=createExercisePreview();
const lesson=createMovementLesson($('movementLesson'));const playerShell=mountExercisePlayer($('player'),{lesson,isBusy:()=>playerBusy()});
...
function renderProgram(){const xs=plan.payload.exercises||[];$('programList').innerHTML='<ul class="loop-preview"></ul>';renderExerciseOverview($('programList').querySelector('ul'),{exercises:xs,progress:session&&sessionSourcePlanId===plan.id&&!session.optionId?session.progress:[],onPreview:x=>preview.open(x,{contextId:plan.id+':'+plan.version})});$('programList').querySelectorAll('[data-preview-exercise]').forEach((b,i)=>b.dataset.programPreview=String(i));}
...
async function start(){preview.close();const xs=exercises();if(!xs.length||starting||playerBusy()||recoveryBlocked||reflectionForm.isBusy())return;starting=true;$('start').disabled=true;try{if(session?.completedAt&&unsynced){$('sync').textContent='Synka det avslutade passet innan du börjar ett nytt.';return}if(!session||session.completedAt){const fresh=await api.patientBootstrap();let checked=false;if(fresh.plan.id!==plan.id){if(pendingEvaluation){checked=await evaluateNext()!==false;if(!checked){$('sync').textContent='Nästa steg kunde inte kontrolleras. Försök igen innan du startar ett nytt pass.';return}}if(!checked||plan.id!==fresh.plan.id){await bootstrap();$('sync').textContent='Din behandlare har uppdaterat planen. Läs den nya versionen innan du startar passet.';return}}if(!checked&&await evaluateNext()===false){$('sync').textContent='Nästa steg kunde inte kontrolleras. Försök igen innan du startar ett nytt pass.';return}session=null;session=S.create(exercises(),crypto.randomUUID(),new Date().toISOString());sessionSourcePlanId=plan.id;if(selectedOption)session.optionId=selectedOption;index=0;await sync('started')} $('start').textContent='Fortsätt passet';$('sessionReflection').hidden=true;$('player').classList.remove('hidden');$('app').dataset.playerOpen='true';renderOptions();showExercise();$('exName').focus({preventScroll:true});$('player').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})}catch(e){$('sync').textContent=e.message||'Planen kunde inte kontrolleras. Försök igen.'}finally{starting=false;$('start').disabled=recoveryBlocked}}
function playerBusy(){return marking||finalizing||!!coach?.isBusy()}
const playerLocks=new Set(),lockedControls=new Map();
function lockPlayer(reason,value){
 if(value){playerLocks.add(reason);lesson.pause();for(const b of document.querySelectorAll('#player button:not(#exerciseCoach button),[data-tab],#logout,#start')){if(!lockedControls.has(b))lockedControls.set(b,b.disabled);b.disabled=true}}
 else{playerLocks.delete(reason);if(!playerLocks.size){for(const [b,disabled]of lockedControls)if(b.isConnected)b.disabled=disabled;lockedControls.clear()}}
 $('player').setAttribute('aria-busy',String(playerLocks.size>0));
}
function showExercise(){
 pauseMotion();const xs=exercises(),x=xs[index],p=session.progress[index],rounds=S.rounds(x),total=session.progress.reduce((n,p)=>n+p.totalRounds,0),done=session.progress.reduce((n,p)=>n+p.roundsDone,0);
 $('sessionProgress').max=total||1;$('sessionProgress').value=done;$('sessionProgressText').textContent=done+' av '+total+' omgångar registrerade';
 $('pos').textContent=`${index+1} av ${xs.length}`;$('exName').textContent=x.name;
 $('dose').textContent=doseLabel(x)+(x.side==='both'?' per sida':x.side==='left'?' · vänster':x.side==='right'?' · höger':'')+(x.prescribedLoad?' · '+x.prescribedLoad:'');
 $('instruction').textContent=instruction(x);
 ...
 $('rounds').innerHTML=rounds.map((r,i)=>`<button class="btn ${i<p.roundsDone?'primary':'soft'} round" data-round="${i}" ${i!==p.roundsDone||p.status==='skipped'?'disabled':''}>${i<p.roundsDone?'✓ ':''}Omgång ${r.set} klar${r.side==='left'?' · vänster':r.side==='right'?' · höger':''}</button>`).join('');
 document.querySelectorAll('[data-effort]').forEach(b=>{b.setAttribute('aria-pressed',String(b.dataset.effort===p.feedback));b.onclick=()=>changeProgress(s=>({...s,progress:s.progress.map((r,i)=>i===index?{...r,feedback:b.dataset.effort}:r)}),{endRest:false})});
 lesson.setExercise(x,{motionKey:motionKey(x),exerciseSide:currentSide(x),contextId:plan.id+':'+plan.version+':'+(session.optionId||'default')});coach?.setExercise({plan:sessionPlan(),exercise:x,sessionId:session.id});
 document.querySelectorAll('[data-round]').forEach(b=>b.onclick=()=>markRound(Number(b.dataset.round)));
 $('player').dataset.complete=String(p.status==='completed'||p.status==='skipped');$('next').disabled=!['completed','skipped'].includes(p.status);$('next').textContent=index===xs.length-1?'Avsluta pass':'Nästa övning';
 playerShell.updateActions({exercise:x,progress:p,index,rows:session.progress});
}
async function changeProgress(change,{rest=false,endRest=true}={}){
 if(!session||session.completedAt||playerBusy())return;marking=true;
 try{playerShell.closePanels();if(endRest)playerShell.endRest();session=change(session);showExercise();if(rest&&session.progress[index].roundsDone<sets(exercises()[index]))playerShell.startRest(Number(exercises()[index].dose?.rest));lockPlayer('saving',true);await sync('partial')}
 finally{marking=false;lockPlayer('saving',false)}
}
async function markRound(i){if(!session||i!==session.progress[index].roundsDone||session.progress[index].status==='skipped')return;await changeProgress(s=>S.mark(s,index,i),{rest:true})}
async function skip(){if(!session||session.progress[index].roundsDone>=session.progress[index].totalRounds)return;await changeProgress(s=>S.skip(s,index));if(!playerBusy())$('next').focus({preventScroll:true})}
async function undoRound(){if(!session?.progress[index]?.roundsDone)return;await changeProgress(s=>S.undo(s,index));if(!playerBusy())$('rounds').querySelector('button:not(:disabled)')?.focus({preventScroll:true})}
async function resumeSkipped(){if(session?.progress[index]?.status!=='skipped')return;await changeProgress(s=>S.resume(s,index));if(!playerBusy())$('rounds').querySelector('button:not(:disabled)')?.focus({preventScroll:true})}
function moveExercise(nextIndex){
 if(!session||session.completedAt||playerBusy()||nextIndex<0||nextIndex>=exercises().length)return;
 playerShell.closePanels();playerShell.endRest();index=nextIndex;showExercise();$('exName').focus({preventScroll:true});$('player').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});if(unsynced)persist();
}
async function finish(){
 if(!session||session.completedAt||playerBusy())return;finalizing=true;playerShell.closePanels();playerShell.endRest();lockPlayer('finishing',true);
 try{
  session=S.finish(session,new Date().toISOString());const all=session.status==='completed',finishedId=session.id,saved=await sync(session.status,session.completedAt);
  pauseMotion();$('player').classList.add('hidden');$('app').dataset.playerOpen='false';renderToday();$('start').textContent='Starta nytt pass';$('start').focus({preventScroll:true});
 ...
 }finally{finalizing=false;lockPlayer('finishing',false);if($('player').classList.contains('hidden')&&$('sessionReflection').hidden)$('start').focus({preventScroll:true})}
}
async function next(){if(!session||session.completedAt||playerBusy()||$('next').disabled)return;if(index<exercises().length-1){moveExercise(index+1);return}await finish()}
async function pause(){if(!session||session.completedAt||playerBusy())return;playerShell.closePanels();$('player').classList.add('hidden');$('app').dataset.playerOpen='false';pauseMotion();renderToday();$('start').textContent='Fortsätt passet';$('start').focus({preventScroll:true});await sync(session.progress.some(p=>p.roundsDone||p.status==='skipped')?'partial':'started')}
