...
async function pause(){if(!session||session.completedAt||coach?.isBusy())return;playerShell.closePanels();$('player').classList.add('hidden');$('app').dataset.playerOpen='false';pauseMotion();$('start').focus({preventScroll:true});await sync(session.progress.some(p=>p.roundsDone||p.status==='skipped')?'partial':'started')}
