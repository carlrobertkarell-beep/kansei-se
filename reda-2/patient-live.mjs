...
let helpExpanded=null,motionSlow=false,motionPlaying=false,motionFocus=null,motionPosition=0,coach=null;
...
function motionPhase(label,phase){if($('motionPhase'))$('motionPhase').textContent=label;document.querySelectorAll('[data-phase-dot]').forEach(el=>el.dataset.active=String(el.dataset.phaseDot===phase))}
function drawMotion(t){const x=exercises()[index];if(!x)return;motionPosition=t;$('motion').innerHTML=F.svg(motionKey(x),t,currentSide(x)||'simultaneous',x.name,{focus:motionFocus});$('motionSide').textContent=sideLabels[currentSide(x)]||'Sida behöver förtydligas'}
function motionFrame(t){pauseMotion();drawMotion(t);document.querySelectorAll('[data-motion-frame]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.motionFrame)===t)));motionPhase(t===0?'Startläge':t===1?'Slutläge':'På väg',t===0?'start':t===1?'end':'out')}
function playMotion(){if(motionPlaying){pauseMotion();return}const x=exercises()[index];if(!x)return;
 // Tutorial playback speed is separate from prescribed tempo, hold time and recorded repetitions.
 stopMotion=F.animate($('motion'),motionKey(x),{seconds:motionSlow?14:7,side:currentSide(x),onPhase:motionPhase,onFrame:t=>{motionPosition=t}});motionPlaying=true;$('motionPlay').textContent='Pausa rörelsen';motionPhase('Rörelsevisning','out');document.querySelectorAll('[data-motion-frame]').forEach(b=>b.setAttribute('aria-pressed','false'))}
...
coach=createExerciseCoach($('exerciseCoach'),{api,onDemonstrate:(frame,focus,scroll=false)=>{motionFocus=focus;motionFrame(frame===null?motionPosition:frame);if(scroll){$('motionStudio').focus({preventScroll:true});$('motionStudio').scrollIntoView({block:'center',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'})}},onSaved:()=>{support?.refresh();barriers?.refresh()},onBusy:value=>{
 if(value){for(const b of document.querySelectorAll('#player button:not(#exerciseCoach button),[data-tab],#logout')){coachDisabled.set(b,b.disabled);b.disabled=true}}
