import {startHTML} from './first-patient-steps.mjs?v=1'
...
function renderToday(){
 renderOptions();const view=todayState(plan,state.sessions);$('todayCard').dataset.dayState=view.kind;$('todayTitle').textContent=view.title;$('todayDetail').textContent=view.detail;
 $('todayExercises').innerHTML=exercises().map(x=>'<li>'+esc(x.name)+' · '+esc(doseLabel(x))+'</li>').join('');
...
async function start(){const xs=exercises();if(!xs.length||starting||recoveryBlocked||reflectionForm.isBusy())return;starting=true;$('start').disabled=true;try{if(session?.completedAt&&unsynced){$('sync').textContent='Synka det avslutade passet innan du börjar ett nytt.';return}if(!session||session.completedAt){const fresh=await api.patientBootstrap();if(fresh.plan.id!==plan.id){await bootstrap();$('sync').textContent='Din behandlare har uppdaterat planen. Läs den nya versionen innan du startar passet.';return}if(await evaluateNext()===false){$('sync').textContent='Nästa steg kunde inte kontrolleras. Försök igen innan du startar ett nytt pass.';return}session=null;session=S.create(exercises(),crypto.randomUUID(),new Date().toISOString());if(selectedOption)session.optionId=selectedOption;index=0;await sync('started')} $('start').textContent='Fortsätt passet';$('sessionReflection').hidden=true;$('player').classList.remove('hidden');renderOptions();showExercise()}catch(e){$('sync').textContent=e.message||'Planen kunde inte kontrolleras. Försök igen.'}finally{starting=false;$('start').disabled=recoveryBlocked}}
