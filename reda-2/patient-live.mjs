...
async function bootstrap(){
 state=await api.patientBootstrap();plan=state.plan;selectedOption=null;
...
async function start(){const xs=exercises();if(!xs.length||starting||recoveryBlocked||reflectionForm.isBusy())return;starting=true;$('start').disabled=true;try{if(session?.completedAt&&unsynced){$('sync').textContent='Synka det avslutade passet innan du börjar ett nytt.';return}if(!session||session.completedAt){if(await evaluateNext()===false){$('sync').textContent='Nästa steg kunde inte kontrolleras. Försök igen innan du startar ett nytt pass.';return}session=null;session=S.create(exercises(),crypto.randomUUID(),new Date().toISOString());if(selectedOption)session.optionId=selectedOption;index=0;await sync('started')} $('start').textContent='Fortsätt passet';$('sessionReflection').hidden=true;$('player').classList.remove('hidden');renderOptions();showExercise()}finally{starting=false;$('start').disabled=recoveryBlocked}}
