...
import {planChangeNotice} from './patient-ei-notice.mjs?v=20260919-1';
...
async function bootstrap(){
 state=await api.patientBootstrap();accountUI.update({name:state.patient.display_name,signedIn:true});plan=state.plan;selectedOption=null;planMessage='';if(api.openPatientPlan)try{const h=await api.openPatientPlan(plan.id);planMessage=h.message||''}catch{planMessage='Planen visas, men kliniken har ännu inte fått kvitto på att du öppnat den.'}
...
async function finishSession(){
 ...
 try{
  ...
  if(saved){try{state=await api.patientBootstrap();plan=state.plan;renderHome();barriers?.refresh();$('sync').textContent=all?'Passet är sparat och synkat':overview.done?'Passet är sparat som delvis genomfört':'Passet är avslutat utan registrerade omgångar';openReflection(pendingReflections(plan,state.sessions,state.reflections).find(x=>x.client_session_id===finishedId))}catch{$('sync').textContent='Passet är sparat, men historiken kunde inte uppdateras.'}}
