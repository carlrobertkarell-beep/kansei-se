...
async function pause(){if(!session||session.completedAt)return;$('player').classList.add('hidden');$('app').dataset.playerOpen='false';stopMotion?.();$('start').focus({preventScroll:true});await sync(session.progress.some(p=>p.roundsDone||p.status==='skipped')?'partial':'started')}
...
$('logout').onclick=async()=>{if(responseForm.isBusy()||reflectionForm.isBusy()||messages?.isBusy()||checkin?.isBusy()||barriers?.isBusy()){$('sync').textContent='Vänta tills återkopplingen har sparats eller ett felmeddelande visas.';return}if(unsynced){$('sync').textContent='Synka passet innan du loggar ut.';return}R.clear(localStore(),state?.userId);await api.signOut();location.reload()};$('start').onclick=start;$('skip').onclick=skip;$('next').onclick=next;$('closePlayer').onclick=pause
document.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{if(b.dataset.tab!=='today')pauseMotion();if(b.dataset.tab==='activity'&&state){activity?.destroy();activity=mountActivityLog($('activityContent'),{api,patientId:state.patient.id})}document.querySelectorAll('[data-tab]').forEach(x=>{x.classList.toggle('active',x===b);if(x===b)x.setAttribute('aria-current','page');else x.removeAttribute('aria-current')});document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('hidden',x.id!==b.dataset.tab))})
...
$('retrySync').onclick=retry;window.addEventListener('beforeunload',e=>{if((unsynced&&!durable)||reflectionForm.isBusy()||checkin?.isBusy()||barriers?.isBusy()){e.preventDefault();e.returnValue=''}});
