...
export function createAuthoringTools(P,D){
 ...
 function replace(plan,index,id){const p=cleanProgression(plan),old=p.exercises[index];if(!old)throw Error('Övningen saknas i planen.');if(p.exercises.some((x,i)=>i!==index&&x.id===id))throw Error('Ersättaren finns redan i planen.');const replacement=exercise(id,p.context);const allowed=P.allowedSides(replacement.id,replacement.variantId);if(allowed.includes(old.side))replacement.side=old.side;p.exercises[index]=replacement;return recheck(p)}
 ...
 return {compatible,exercise,add,replace,remove,move,fromTemplate,adapt,recheck};
}
