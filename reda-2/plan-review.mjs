import {barrierCandidates} from './barrier-loop.mjs?v=1';
...
export function planCandidates(detail,base,P,D,loopId=null){
 if(loopId||detail.barriers?.some(l=>!['helped','handled','superseded'].includes(l.phase)))return barrierCandidates(detail,base,P,D,loopId);
...
export function changeRows(before,after){
 ...
 for(const [key,label]of [['minutes','Tid tillgänglig'],['equipment','Träningsplats'],['band','Gummiband'],['floorOK','Golvövningar']])if(before?.context?.[key]!==after?.context?.[key]){const format=v=>v===undefined?'Inte angivet':typeof v==='boolean'?(v?'Ja':'Nej'):key==='minutes'?v+' minuter':v==='home'?'Hemma':v==='gym'?'Gym':String(v);rows.push({name:'Förutsättningar',label,before:format(before?.context?.[key]),after:format(after?.context?.[key])});}
...
export async function openPlanReview({api,patientId,payload=null,email='',loopId=null,onBusy,onSaved}){
 ...
 function draw(){dialog.innerHTML='<header><div><p class="eyebrow">Granska och lämna över</p><h2>'+esc(detail.patient.display_name)+'</h2></div><button class="btn ghost" data-close>Stäng</button></header><p>'+esc(base?'Den aktuella planen är v'+detail.patient.plan_version+'. Du godkänner en ny version.':'Patientens första digitala plan.')+'</p>'+(choices.length>1?'<label>EI:s förslag<select data-choice>'+choices.map((c,i)=>'<option value="'+i+'">'+esc(c.title)+'</option>').join('')+'</select></label>':'')+'<h3 data-title></h3><p data-why></p><div data-diff></div><details><summary>Övningar, instruktioner och dos · justera vid behov</summary><div data-exercises></div></details><p><b>Uppföljning:</b> '+esc(careText(chosen.plan))+'</p>'+(chosen.loopId?'<section class="barrier-review"><h3>Följ upp om ändringen hjälper</h3><p>Efter ett registrerat pass med den nya planen frågar Reda om '+(chosen.barrier==='time'?'tiden fungerade':'platsen och utrustningen fungerade')+'. Ett positivt svar kan avsluta det praktiska hindret. Kontaktönskemål, andra signaler och nya besvär behöver fortfarande din bedömning.</p><label>Följ upp om svar saknas<input data-barrier-due type="date" min="'+detail.today+'" value="'+new Date(Date.parse(detail.today+'T12:00:00Z')+7*86400000).toISOString().slice(0,10)+'"></label></section>':'')+'<label>Din bedömning<textarea data-note maxlength="1000" placeholder="Kort motivering till din bedömning"></textarea></label><label>Texten patienten ser<textarea data-message maxlength="500">'+esc(chosen.loopId?(chosen.barrier==='time'?'Vi har anpassat planen efter tiden du har. Prova det uppdaterade passet, så följer vi upp om det blev lättare att hinna.':'Vi har anpassat planen efter din plats och utrustning. Prova det uppdaterade passet, så följer vi upp om det fungerade.'):base?'Din plan är uppdaterad. Se ändringarna i övningarna innan du startar nästa pass.':'Här är planen vi har förberett för dig. Börja med att läsa övningarna och starta ditt första pass när det passar enligt träningsdagarna.')+'</textarea></label>'+(!detail.patient.connected?'<label>Patientens e-postadress<input data-email type="email" required autocomplete="off" value="'+esc(email||detail.contact?.email||'')+'"></label>':'')+'<label class="review-confirm"><input type="checkbox" data-reviewed>Jag har granskat mottagare, övningar, instruktioner, sida, dos och uppföljning.</label><p data-issues role="alert"></p>'+(!detail.delivery_enabled?'<p class="notice">Patientdistribution är stängd tills klinisk granskning och e-postprov är klara. Du kan granska förslaget och spara ett utkast.</p>':'')+'<p data-status role="status"></p><footer><button class="btn ghost" data-draft>Spara som utkast</button><button class="btn primary" data-publish>Godkänn och lämna över</button></footer>';
  ...
  q('[data-publish]').onclick=publish;for(const selector of ['[data-email]','[data-message]','[data-barrier-due]'])q(selector)?.addEventListener('input',()=>{q('[data-reviewed]').checked=false;pending=null});
 }
 async function publish(){
  ...
  const input={...(chosen.loopId?{loopId:chosen.loopId,dueDate:q('[data-barrier-due]').value}:{}),patientId,token:detail.token,basePlanId:detail.patient.plan_id||null,payload:chosen.plan,note,patientMessage,email:mail,decisionId:chosen.decisionId||null},signature=JSON.stringify(input);
  if(pending?.signature!==signature)pending={id:crypto.randomUUID(),signature};lock(true);
  try{receipt=await (chosen.loopId?api.publishBarrier:api.publishReviewed)({...input,requestId:pending.id});pending=null;
   let sent=receipt.delivery_state==='not_requested';if(!sent)try{await api.sendInvitation(receipt.id);sent=true}catch{}
   const result=receipt;dialog.innerHTML='<h2>Plan v'+esc(result.version)+' är publicerad</h2><p>'+esc(sent?(mail?'Inbjudan har lämnats till e-posttjänsten. Öppnad plan och första pass syns i patientöversikten.':'Patienten ser den nya planen vid nästa hämtning i Reda.'):'Planen är publicerad, men utskicket är inte bekräftat. Försök skicka igen från patientöversikten. Ingen ny plan behövs.')+'</p><p>'+esc(chosen.loopId?'Reda följer upp ändringen efter ett registrerat pass. Uteblivet eller otillräckligt svar ligger kvar för din bedömning.':'Öppna patientsignaler ligger kvar tills du dokumenterar att de är bedömda.')+'</p><button class="btn primary" data-done>Klart</button>';q('[data-done]').onclick=close;try{await onSaved?.(result)}catch{if(alive)dialog.querySelector('p').textContent+=' Överblicken kunde inte uppdateras. Stäng och hämta patienten igen; publiceringen är sparad.'};
 ...
 try{detail=await api.dashboardPatient(patientId);if(!payload&&detail.decision?.action==='advance'&&api.evaluateProgression){await api.evaluateProgression(patientId,crypto.randomUUID());if(!alive)return;detail=await api.dashboardPatient(patientId)}const summary=await api.patientSummary(patientId);if(!alive)return;const active=summary.plans.find(p=>p.status==='active');if((active?.id||null)!==(detail.patient.plan_id||null))throw Error('Planen har ändrats. Stäng och öppna granskningen igen.');base=active?.payload||null;
  choices=payload?[{id:'manual',title:base?'Din granskade planändring':'Första planen',detail:'Granska det samlade innehållet och mottagaren före överlämning.',plan:structuredClone(payload)}]:planCandidates(detail,base,window.RedaPlanner,window.RedaData,loopId);
