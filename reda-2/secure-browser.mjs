...
export async function patientBootstrap(){const u=await currentUser();if(!u)throw new Error('Logga in först');await rpc('reda_claim_patient_access',{},db);const {data:patient,error:pErr}=await db.from('reda_patients').select('id,display_name').eq('auth_user_id',u.id).eq('status','active').maybeSingle();if(pErr)throw pErr;if(!patient)throw Object.assign(new Error('Ingen aktuell patientinbjudan hittades för det här kontot. Kontrollera att du använder samma e-postadress som du gav kliniken.'),{code:'NO_PATIENT'});const {data:plan,error}=await db.from('reda_plans').select('id,version,payload,activated_at').eq('patient_id',patient.id).eq('status','active').maybeSingle();if(error)throw error;if(!plan)throw Object.assign(new Error('Du är inloggad, men ingen aktiv plan finns ännu. Kontakta kliniken eller försök hämta planen igen.'),{code:'NO_PLAN'});const {data:sessions,error:sErr}=await db.from('reda_sessions').select('id,client_session_id,plan_id,status,started_at,completed_at,plan_version,payload').eq('patient_id',patient.id).order('started_at',{ascending:false}).limit(30);if(sErr)throw sErr;const [{data:history,error:hErr},{data:decisions,error:dErr}]=await Promise.all([db.from('reda_plans').select('id,version,payload,activated_at,status').eq('patient_id',patient.id).lt('version',plan.version).order('version',{ascending:false}).limit(1),db.from('reda_engine_decisions').select('id,action,code,result_plan_id,created_at').eq('patient_id',patient.id).eq('result_plan_id',plan.id).order('created_at',{ascending:false}).limit(1)]);if(hErr)throw hErr;if(dErr)throw dErr;return {userId:u.id,patient,plan,sessions:sessions||[],planHistory:history||[],latestDecision:decisions?.[0]||null,...await trainingResponses(patient.id,db),...await sessionReflections(patient.id,db)}}
