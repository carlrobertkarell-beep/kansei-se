...
export async function patientBootstrap(){const u=await currentUser();if(!u)throw new Error('Logga in först');const {data:patient,error:pErr}=await db.from('reda_patients').select('id,display_name').eq('auth_user_id',u.id).eq('status','active').single();if(pErr)throw pErr;const {data:plan,error}=await db.from('reda_plans').select('id,version,payload,activated_at').eq('patient_id',patient.id).eq('status','active').single();if(error)throw error;const {data:sessions,error:sErr}=await db.from('reda_sessions').select('id,client_session_id,plan_id,status,started_at,completed_at,plan_version,payload').eq('patient_id',patient.id).order('started_at',{ascending:false}).limit(30);if(sErr)throw sErr;return {userId:u.id,patient,plan,sessions:sessions||[],...await trainingResponses(patient.id,db),...await sessionReflections(patient.id,db)}}
