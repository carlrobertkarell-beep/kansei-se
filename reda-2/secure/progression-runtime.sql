...
-- Durable EI queue. Writes enqueue; a trusted service worker claims and completes jobs separately.
create table public.reda_engine_queue(id bigserial primary key,patient_id uuid not null references public.reda_patients(id),reason text not null check(reason in ('session_closed','response_saved','reflection_saved','manual')),source_id uuid,available_at timestamptz not null default now(),status text not null default 'pending' check(status in ('pending','processing','done','failed')),attempts integer not null default 0,last_error text not null default '',created_at timestamptz not null default now(),updated_at timestamptz not null default now());
create unique index reda_engine_queue_source on public.reda_engine_queue(patient_id,reason,source_id) where source_id is not null;create index reda_engine_queue_pending on public.reda_engine_queue(available_at,id) where status='pending';alter table public.reda_engine_queue enable row level security;revoke all on public.reda_engine_queue from public,anon,authenticated;
create function private.reda_enqueue_engine(p_patient uuid,p_reason text,p_source uuid default null) returns void language plpgsql security definer set search_path='' as $$begin if p_reason not in ('session_closed','response_saved','reflection_saved','manual') then raise exception 'Invalid engine queue reason';end if;insert into public.reda_engine_queue(patient_id,reason,source_id)values(p_patient,p_reason,p_source)on conflict do nothing;end$$;revoke all on function private.reda_enqueue_engine(uuid,text,uuid) from public,anon,authenticated;
create function private.reda_queue_response() returns trigger language plpgsql security definer set search_path='' as $$begin perform private.reda_enqueue_engine(new.patient_id,'response_saved',new.id);return new;end$$;create trigger reda_response_engine_queue after insert on public.reda_training_responses for each row execute function private.reda_queue_response();
create function private.reda_claim_engine_queue(p_limit integer default 25) returns setof public.reda_engine_queue language plpgsql security definer set search_path='' as $$begin if p_limit not between 1 and 100 then raise exception 'Invalid queue batch';end if;return query with picked as(select q.id from public.reda_engine_queue q where q.status='pending' and q.available_at<=now() order by q.available_at,q.id for update skip locked limit p_limit) update public.reda_engine_queue q set status='processing',attempts=q.attempts+1,updated_at=now() from picked where q.id=picked.id returning q.*;end$$;
create function private.reda_finish_engine_queue(p_id bigint,p_ok boolean,p_error text default '') returns void language plpgsql security definer set search_path='' as $$declare n integer;begin update public.reda_engine_queue set status=case when p_ok then 'done' when attempts>=5 then 'failed' else 'pending' end,last_error=left(coalesce(p_error,''),1000),available_at=case when p_ok or attempts>=5 then available_at else now()+make_interval(mins=>least(60,greatest(1,attempts*attempts))) end,updated_at=now() where id=p_id and status='processing';get diagnostics n=row_count;if n<>1 then raise exception 'Queue item unavailable';end if;end$$;
revoke all on function private.reda_claim_engine_queue(integer),private.reda_finish_engine_queue(bigint,boolean,text) from public,anon,authenticated;grant execute on function private.reda_claim_engine_queue(integer),private.reda_finish_engine_queue(bigint,boolean,text) to service_role;
create function public.reda_worker_claim(p_limit integer default 25) returns setof public.reda_engine_queue language sql security definer set search_path='' as $$select * from private.reda_claim_engine_queue(p_limit)$$;
create function public.reda_worker_evaluate(p_queue_id bigint,p_patient_id uuid) returns jsonb language plpgsql security definer set search_path='' as $$declare d jsonb;begin if not exists(select 1 from public.reda_engine_queue where id=p_queue_id and patient_id=p_patient_id and status='processing')then raise exception 'Queue item unavailable';end if;d=private.reda_evaluate_progression(p_patient_id,gen_random_uuid());perform private.reda_finish_engine_queue(p_queue_id,true,'');return d;end$$;
create function public.reda_worker_fail(p_queue_id bigint,p_error text) returns void language sql security definer set search_path='' as $$select private.reda_finish_engine_queue(p_queue_id,false,p_error)$$;
revoke all on function public.reda_worker_claim(integer),public.reda_worker_evaluate(bigint,uuid),public.reda_worker_fail(bigint,text) from public,anon,authenticated;grant execute on function public.reda_worker_claim(integer),public.reda_worker_evaluate(bigint,uuid),public.reda_worker_fail(bigint,text) to service_role;

create function public.reda_worker_health() returns jsonb language sql stable security definer set search_path='' as $$select jsonb_build_object('pending',count(*) filter(where status='pending'),'processing',count(*) filter(where status='processing'),'failed',count(*) filter(where status='failed'),'done_24h',count(*) filter(where status='done' and updated_at>now()-interval '24 hours'),'oldest_pending_seconds',coalesce(extract(epoch from(now()-min(created_at)filter(where status='pending')))::bigint,0),'attempts_24h',coalesce(sum(attempts)filter(where updated_at>now()-interval '24 hours'),0)) from public.reda_engine_queue$$;revoke all on function public.reda_worker_health() from public,anon,authenticated;grant execute on function public.reda_worker_health() to service_role;
