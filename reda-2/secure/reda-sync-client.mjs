...
export function createRedaClient(url,publishableKey){
  ...
  return {
    ...
    async activatePlan(planId){await requireClinician();const {data,error}=await db.functions.invoke('activate-plan',{body:{plan_id:planId}});if(error)throw error;return data},
    ...
    async saveSession(planId,session){const body={client_session_id:session.clientSessionId,plan_id:planId,status:session.status,started_at:session.startedAt,completed_at:session.completedAt||null,payload:session.payload||{}};const {data,error}=await db.functions.invoke('save-session',{body});if(error)throw error;return data},
