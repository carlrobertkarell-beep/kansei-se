...
Deno.serve(async(req:Request)=>{
 ...
 if(Number(req.headers.get('content-length')||0)>6000)return json({error:'Frågan är för lång.'},413);
 const token=req.headers.get('authorization');if(!token?.startsWith('Bearer '))return json({error:'Logga in igen.'},401);
 try{
  const text=await req.text();if(text.length>6000)return json({error:'Frågan är för lång.'},413);const body=JSON.parse(text);
  ...
  const result=await ask({question:body.question,plan:data.plan,model,apiKey,endpoint:Deno.env.get('OPENAI_REDA_ENDPOINT')||'https://api.openai.com/v1/responses'});
  return json(result);
